...
Global effects are card-data-driven, but remain conservative. The current online-audit scope is exactly 65 audited lasting whole-game effects after `CORE_` reprints are canonicalized; it is an explicit reviewed set, not an automatic new-card detector. The engine resolves a card from the local database, then accepts only an audited rule whose mechanics and rules text allow the observed activation: `START_OF_GAME_KEYWORD`, an actual play, a deathrattle trigger, or a mapped triggered effect. A separate legacy structural-card list remains for older structural reminders that cannot be safely inferred from current text. Ordinary board auras, unaudited cards, mismatched trigger types, missing card data, and unknown owners fail closed and do not appear. Confirmed friendly ownership is stored in `globalEffects`; confirmed opponent ownership is stored separately in `opponentGlobalEffects`, so neither side leaks into the other. Effects clear on a real new game and on game end; duplicate same-start records do not clear or double-count the current match. Seeing a card, enchantment, or deathrattle label alone is never enough: the matching reliable activation and ownership must both be present. The sanitized regression fixture under `fixtures/logs/constructed-duplicate-create` preserves the observed duplicate-start and `JAIL_397` trigger shape without retaining account identity.

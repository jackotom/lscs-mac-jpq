...
- `src/main/ladderDeckOverlayController.ts`: shows the left recommendation window only for Standard/Wild while Hearthstone is frontmost, never shows it for Casual, rejects stale mode refreshes, and preserves a manual close until the mode changes.
...
When the newest session only has `Decks.log`, the service publishes the recognized waiting state, loads the local card database before collection-deck preview, and starts constructed-screen recognition. This allows a cold launch directly into Standard, Wild, or Casual deck selection to publish an overlay context before Arena has ever been opened, with mana costs and card thumbnails already available even though `Power.log` does not exist yet.
...
The same local recognizer is also used on Standard/Wild/Casual deck-select screens before a constructed game starts. Standard and Wild apply mode filtering; Casual activates a collection deck only when the selected name resolves to one unique stored deck. This keeps a completed Arena deck from staying on the overlay when the player returns to constructed deck selection.
Constructed deck-select recognition is allowed to run whenever the tracker is not inside an active game. A verified Standard/Wild/Casual screen deck immediately resets the Arena state and previews the constructed collection deck, so leaving Arena or switching between constructed decks on the deck-select screen does not require a fresh `Power.log` write before the overlay switches. A `Power.log` `GT_CASUAL` game start is managed as a constructed tracker session rather than an unsupported mode.
...
Global effects are card-data-driven, but remain conservative. The current online-audit scope is exactly 65 audited lasting whole-game effects after `CORE_` reprints are canonicalized; it is an explicit reviewed set, not an automatic new-card detector. By default, missing local card data fails closed. Two explicit exceptions remain: reviewed enchantment-to-source mappings can publish their known source effect when the mapped enchantment entity and ownership are observed, and the legacy structural-card list can publish older reminders that cannot be safely inferred from current text. Otherwise, the engine accepts only an audited rule whose mechanics and rules text allow the observed activation: `START_OF_GAME_KEYWORD`, an actual play, a deathrattle trigger, or a mapped triggered effect. Ordinary board auras, unaudited cards, mismatched trigger types, and unknown owners do not appear. Confirmed friendly ownership is stored in `globalEffects`; confirmed opponent ownership is stored separately in `opponentGlobalEffects`, so neither side leaks into the other. Effects clear on a real new game and on game end; duplicate same-start records do not clear or double-count the current match. Seeing an arbitrary card, enchantment, or deathrattle label alone is never enough: the matching reliable activation and ownership must both be present. Sanitized regression fixtures under `fixtures/logs/constructed-duplicate-create` and `fixtures/logs/triggered-global-effect-enchantments` preserve the observed duplicate-start, trigger, and permanent-enchantment shapes without retaining account identity.
...
- `cardRelationRules` 只从明确的友方牌库、手牌、使用、施放、下一张牌和友方类别增益语句生成关系；跨句条件、对手或双方牌库、随机生成、洗入、负面构筑条件、动态或相对数值默认拒绝。
- 选择器支持卡牌类型、法力值、攻击、生命、种族、法术派系、关键词和稀有度；并列目标按 OR 分开，同一目标的多个限制按 AND 匹配，融合怪种族 `ALL` 可匹配任一种族。
- 关系只用于牌库目标并支持双向查看；同一 `dbfId`，或去除连续 `CORE_` / `VAN_` 后相同的卡牌编号，不得互相高亮。
- renderer 边界拒绝空选择器、空数组、空数值条件、负数和非有限数，避免损坏载荷高亮整副牌库。
